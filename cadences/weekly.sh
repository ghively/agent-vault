...
python3 -m agent_vault.compiler . || { crc=$?; rc=$crc; detail="$detail compile(rc=$crc)"; }

# 3. Drain discoveries into the registry -- runs regardless of compile outcome.
python3 -m agent_vault.promote .  || { prc=$?; rc=$prc; detail="$detail promote(rc=$prc)"; }

# 4. Validate the resulting tree.
python3 -m agent_vault.validate . >/dev/null || { vrc=$?; rc=$vrc; detail="$detail validate(rc=$vrc)"; }
